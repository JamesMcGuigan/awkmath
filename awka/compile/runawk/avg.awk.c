...
END()
{
  if (COUNT_awk->dval == 0)
  {
    awka_print(NULL, 0, 0, awka_arg1(a_TEMP, _litd0_awka));
    goto __24;
  }
  awka_print(NULL, 0, 0, awka_arg1(a_TEMP, awka_tmp_dbl2var(awka_div(SUM_awk->dval, COUNT_awk->dval))));
  __24:;
...
MAIN()
{
  ...
  while (awka_getline(a_TEMP, awka_dol0(0), awka_gets(a_bivar[a_FILENAME]), FALSE, TRUE)->dval > 0 && awka_setNF())
  {
    if (awka_vartrue(isnumber_fn(awka_arg1(a_TEMP, awka_doln(1, 0)))))
    {
      ...
      awka_setd(SUM_awk) += awka_getd(awka_doln(1, 0));
    }
    nextrec:;
  }
}
...
main(int argc, char *argv[])
{
  ...
  awka_initgvar(1, "SUM_awk", SUM_awk);
  _gvar[2].name = NULL;
  _gvar[2].var  = NULL;
